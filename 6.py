...
def extract_markdown_from_pdf_llm(pdf_file) -> str:
    """使用高质量 LLM 解析器将 PDF 转换为 Markdown"""
    try:
        import pymupdf4llm
        import tempfile
        import os
...
st.markdown('<p class="sub-header">AI-Powered Fair Recruitment Dashboard | 公平招聘系统 (GPA: 4.0 Scale)</p>',
            unsafe_allow_html=True)


# =============================
# DATA LOADING（公平版数据生成）
# =============================
@st.cache_data
def load_data():
    ...
    if should_generate:
        ...
        # 5. Teaching Experience（从毕业年份自动推导）
        ...
        # 4分制映射: P=2.0, C=2.5, D=3.0, HD=4.0
        gpa_mapping = {'P': 2.0, 'C': 2.5, 'D': 3.0, 'HD': 4.0}
        ...
        df = pd.DataFrame({
            ...
            "gpa": gpa_numeric.astype(float),  # 确保是浮点数
        ...
        print("✅ 公平招聘模型 - 数据生成报告")
        print("=" * 70)
        print(f"总候选人数: {N_RECORDS:,}")
        print(f"整体录取率: {selected.mean():.2%}")
        ...
        print(f"   标准差: {gender_variance:.4f} {'✓ 公平' if gender_variance < 0.02 else '⚠️ 存在差异'}")
        ...
        print(f"   标准差: {eth_variance:.4f} {'✓ 公平' if eth_variance < 0.03 else '⚠️ 存在差异'}")
        ...
        print(f"   标准差: {major_variance:.4f} {'✓ 公平' if major_variance < 0.03 else '⚠️ 存在差异'}")
...
✅ 改进要点:
  1. ❌ 移除专业歧视 - 所有专业权重为 0
  2. ⬆️  提高沟通能力权重 - 0.6（教师核心技能）
  3. ➕ 加入领导力评分 - 0.3（课堂管理能力）
  4. ⬇️  降低GPA权重 - 0.5（不是唯一标准）
  5. 🚫 性别和族裔不影响录取决策
  6. 📊 录取率控制在合理水平（10-20%）

🎯 核心原则: 按个人能力评估,而非按群体标签
        """)

        df.to_csv(file_path, index=False, encoding="utf-8-sig")
        st.success(f"✅ 公平数据集已生成: {file_path}")
...
with tab1:
    ...
    with subtab1:
        ...
        if uploaded_file is not None and analyze_btn:
            with st.spinner("Analyzing resume..."):
                ...
                if parsed_data:
                    st.session_state.pdf_candidate = parsed_data  # 使用独立的key
    ...
    with col_input:
        ...
        with col_btn1:
            if st.button("🎲 Generate Random", use_container_width=True):
                st.session_state.candidate = {
                    ...
                    'gpa': round(np.random.uniform(5.0, 9.0), 1),
        ...
        with col_btn2:
            if st.button("🔄 Reset to Default", use_container_width=True):
                st.session_state.candidate = {
                    ...
                    'degree_major': "Education", 'gpa': 6.5, 'teaching_experience': "1-3 years",
        ...
        with col2:
            gpa = st.slider("GPA (10-point scale)", 1.0, 10.0, float(c['gpa']), 0.1)
    ...
    with col2:
        ...
        # 使用手动输入的候选人数据
        if 'manual_candidate' in st.session_state:
            mc = st.session_state.manual_candidate
            scores = {
